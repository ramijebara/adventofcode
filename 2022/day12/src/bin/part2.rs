...
fn main() {
    ...
    let starting_points: HashMap<Node, i32> =
        map.clone().into_iter().filter(|(_, v)| v == &0).collect();

    let mut starting_point_costs: Vec<i32> = Vec::new();

    for (k, _) in starting_points {
        if let Some(path) = shortest_path(k, &end, &map) {
            starting_point_costs.push(path.len);
    ...
    if let Some(min_cost) = starting_point_costs.iter().min() {
        println!("Result: {}", min_cost);
    } else {
        error!("Can't find a low cost path to destination");
    }
}
